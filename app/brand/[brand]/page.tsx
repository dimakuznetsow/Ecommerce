...
export default function Category({ searchParams }: SearchParamType) {
    ...
    const filteredProducts = products?.filter(product => product.metadata.brand === searchParams.brand)
    ...
    if (filteredProducts.length === 0) {
        return (
            <main className='mx-4 lg:mx-48 justify-center mb-10'>
                <div className="text-xl font-black">NO MATCHES FOR "{searchParams.brand?.toUpperCase()}"</div>
            </main>
        )
    }
            ...
            <main >
                <nav className="flex justify-between items-center">
                    <h1 className='mx-4 lg:mx-48 mb-6 text-2xl md:text-3xl font-black'>
                        {searchParams.brand?.toUpperCase().replace(/1/g, " ")}
                    </h1>
                    <select
